...
class Annotation
{
     ...
     */
    public static function arrayValue($annotation, $settings = [])
    {
        ...
        // load from config
        if ($openingBrace === '>') {
            preg_match('/\w+/', $annotation, $field);
            return isset($settings['modules']['config']['CustomData'][$field[0]]) ? $settings['modules']['config']['CustomData'][$field[0]] : null;
        }
        return null;
    }
}
